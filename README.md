...
- **[Getting In Is the Easy Part](./agent-exit-problem-long-running-agents.md)** - Agents now run for days, and almost none of them know how to come back — the stranded-state problem, why return planning is invisible until it matters, registering the undo alongside the action, and keeping a two-way line open for the life of a run
- **[Power You Can't Withdraw Isn't Authority](./reversibility-undo-problem-agent-autonomy.md)** - Why reversibility, not permission, is the real ceiling on agent autonomy — the three classes of agent action, why compensation isn't reversal, and the design rule of registering the undo before granting the power
- **[The Agent That Remembers: Why Persistent Memory Is the Next Trust Boundary](./agent-memory-persistent-state-trust-boundary.md)** - How persistent memory turns agents into entities with history — memory poisoning as time-delayed prompt injection, consolidation drift, and why the unit of audit must become the memory transaction rather than the model version
- **[Agent Identity: OAuth Was Built for Humans — What Works for Machines?](./agent-identity-oauth-built-for-humans.md)** - Why human-centric identity protocols break down for autonomous agents, and what machine-native identity, delegation, and capability scoping actually require
- **[Everyone Is Trapped: The Circular Dependency Nobody in AI Wants to Talk About](./everyone-is-trapped-circular-dependency-ai.md)** - The circular trap between model providers, platform builders, developers, and customers — what depreciates in months vs what compounds over years, the 80/20 inversion, and the durability test every AI company should run
- **[The Dharma Machine: Ancient Indian Wisdom & Governing AI](./the-dharma-machine-ancient-indian-wisdom-governing-ai.md)** - What 5,000 years of Indian systems thinking — Ṛta, Karma, Dharma, the Yamas & Niyamas, Guru-Śiṣya lineage, Indra's Net, and Yakṣa-praśna — teaches us about governing autonomous AI agents, with a complete 10-layer Aṣṭāṅga AI Trust Stack
- **[Eval-Driven Development: Building AI Pipelines That Judge Themselves](./eval-driven-development-ai-pipeline-judges.md)** - Why shipping AI without evals is shipping without tests, and how LLM-as-judge pipelines, rubrics, and continuous scoring turn subjective quality into an engineering signal
