...
- 🆕 **[The Agent That Remembers: Why Persistent Memory Is the Next Trust Boundary](./agent-memory-persistent-state-trust-boundary.md)** *(July 2026)* - Always-on agents carry memory across sessions — and that memory is simultaneously their identity, their attack surface, and their audit gap. Memory poisoning as prompt injection with a time delay, drift without attackers, classifier context rot, and the governance primitives to build now: provenance ledgers, consolidation gates, drift baselines, memory rollback, and memory passports. Grounded in the June–July 2026 arXiv research wave (arXiv:2606.30306, 2607.02514, 2607.02374, 2607.01988, and more — full citations in the article)
- **[Agent Identity: OAuth Was Built for Humans — What Works for Machines?](./agent-identity-oauth-built-for-humans.md)** - When an autonomous agent calls your API at 3am, who clicked "Allow"? Why OAuth, API keys, and service accounts all break for agents — and the four architectural patterns (capability tokens, agent cards, cryptographic provenance, trust-through-audit) emerging to replace them
- **[Eval-Driven Development: Why Your AI Pipeline Needs a Judge Before a Deployer](./eval-driven-development-ai-pipeline-judges.md)** - Why "it looks good" isn't engineering — how deterministic evals, LLM-as-Judge, Process Reward Models, and automated red-teaming form a CI/CD pipeline for AI that catches hallucinations, reward hacking, and classifier context rot before production does
